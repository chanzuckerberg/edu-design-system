...
EDS uses [tailwind utility classes](https://v3.tailwindcss.com/docs/), (e.g., `mb-0` and `p-0`) inline in `*.stories.tsx` files to demonstrate allowed compositions and example implementations.
...
We currently use the [`useId` hook](https://reactjs.org/docs/hooks-reference.html#useid) for ID generation (available in React >=18), which can also be used in custom components as well.
